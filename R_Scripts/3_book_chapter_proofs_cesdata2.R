library(here)
source(here("R_Scripts/1_master_file_cesdata2.R"))
...
ces$election
ces %>% 
  filter(election>1978&election< 2021) %>% 
  count(election, vote2, Occupation) %>% 
  write.csv(file="election_vote_from_cesdata2.csv")
ces %>% 
...
filter(vote2!="Green") %>% 
  ...
  ggplot(., aes(x=election, y=pct*100, linetype=Occupation))+
  geom_line()+
  facet_wrap(~vote2)+
...
scale_x_date(breaks= seq.Date(from=as.Date("1980-01-01"), to=as.Date("2021-01-01"), by="4 years"), date_labels="%Y")+
 ...
 ggsave(here("Plots", "book_occupation_share_per_party.eps"), width=5)
...
ces %>% 
  ...
  theme(axis.text.x=element_text(angle=90))+
  scale_x_date(breaks= seq.Date(from=as.Date("1980-01-01"), to=as.Date("2021-01-01"), by="5 years"), date_labels="%Y")->p
...
ggsave(here("Plots", "book_party_share_by_occupation.eps"), width=5)
...
ggsave(here("Plots", "Proportion_working_class_supporting_measures.eps"), width=4.25)
...
ggsave(here("Plots", "book_mip_change.eps"), width=4.25)
