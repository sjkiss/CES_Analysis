...
ces %>% 
  filter(election!=1965 & election!=1972) %>%
  nest(variables=-election) %>% 
  mutate(model=map(variables, function(x) lm(ndp~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+degree+degree:sector+sector, data=x)),
         tidied=map(model, tidy), 
...
ces %>% 
  filter(election!=1965 & election!=1972) %>%
  nest(variables=-election) %>% 
  mutate(model=map(variables, function(x) lm(conservative~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+degree+degree:sector+sector, data=x)), 
         tidied=map(model, tidy),
         vote=rep('Conservative', nrow(.))  
...
ces %>% 
  filter(election!=1965 & election!=1972) %>%
  nest(variables=-election) %>% 
  mutate(model=map(variables, function(x) lm(liberal~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+degree+degree:sector+sector, data=x)), 
         tidied=map(model, tidy),
         vote=rep('Liberal', nrow(.))  
...
#### M16 Blais Replication Extension (including 2019) with Greens ####
table(ces$election, ces$sector)
...
ces %>% 
  ...
  mutate(model=map(variables, function(x) lm(ndp~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+sector+degree+income, data=x)),
         tidied=map(model, tidy), 
         vote=rep('NDP', nrow(.)))->ndp_models_complete16
...
ces %>% 
  ...
  mutate(model=map(variables, function(x) lm(conservative~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+sector+degree+income, data=x)), 
         tidied=map(model, tidy),
         vote=rep('Conservative', nrow(.))  
  )->conservative_models_complete16
...
ces %>% 
  ...
  mutate(model=map(variables, function(x) lm(liberal~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+sector+degree+income, data=x)), 
         tidied=map(model, tidy),
         vote=rep('Liberal', nrow(.))  
  )->liberal_models_complete16

ces %>% 
  filter(election!=1965 & election!=1972) %>%
  nest(variables=-election) %>% 
  mutate(model=map(variables, function(x) lm(green~as.factor(region2)+catholic+no_religion+non_charter_language+working_class+union_both+age+female+sector+degree+income, data=x)),
         tidied=map(model, tidy), 
         vote=rep('NDP', nrow(.)))->green_models_complete16

stargazer(ndp_models_complete16$model, 
          type="html", 
          out=here("Tables", "NDP_Models_1968_2019_1.html"),
          column.labels=c("1968", "1974", "1979", "1980", "1984", "1988", "1993", "1997", "2000", "2004", "2006", "2008", "2011", "2015", "2019"), 
          star.cutoffs=c(0.05), 
...
stargazer(liberal_models_complete16$model, 
          type="html", 
          out=here("Tables", "liberal_Models_1968_2019_1.html"),
          column.labels=c("1968", "1974", "1979", "1980", "1984", "1988", "1993", "1997", "2000", "2004", "2006", "2008", "2011", "2015", "2019"), 
          star.cutoffs=c(0.05), 
...
stargazer(conservative_models_complete16$model, 
          type="html", 
          out=here("Tables", "conservative_Models_1968_2019_1.html"),
          column.labels=c("1968", "1974", "1979", "1980", "1984", "1988", "1993", "1997", "2000", "2004", "2006", "2008", "2011", "2015", "2019"), 
          star.cutoffs=c(0.05), 
...
stargazer(green_models_complete16$model, 
          type="html", 
          out=here("Tables", "Green_Models_1968_2019_1.html"),
          column.labels=c("1968", "1974", "1979", "1980", "1984", "1988", "1993", "1997", "2000", "2004", "2006", "2008", "2011", "2015", "2019"), 
          star.cutoffs=c(0.05), 
          title="Green Models 1968-2019", 
          notes=paste("Printed on", as.character(Sys.time()), "by", Sys.getenv("USERNAME")))

#### Blais Plots ####
# Set theme
theme_set(theme_bw())
ndp_models_complete16 %>% 
  unnest(tidied) %>% 
  filter(term=="sector") %>% 
  ggplot(., aes(x=election, y=estimate))+geom_point()+labs(title="OLS Coefficients of Public Sector on NDP Vote")+geom_errorbar(aes(ymin=estimate-(1.96*std.error), ymax=estimate+(1.96*std.error)), width=0)+ylim(c(-0.2,0.2))
#ggsave(here("Plots", "sector_ndp_1968_2019_16.png"))

liberal_models_complete16 %>% 
  unnest(tidied) %>% 
  filter(term=="sector") %>% 
  ggplot(., aes(x=election, y=estimate))+geom_point()+labs(title="OLS Coefficients of Public Sector on Liberal Vote")+geom_errorbar(aes(ymin=estimate-(1.96*std.error), ymax=estimate+(1.96*std.error)), width=0)+ylim(c(-0.2,0.2))
#ggsave(here("Plots", "sector_liberal_1968_2019_16.png"))

conservative_models_complete16 %>% 
  unnest(tidied) %>% 
  filter(term=="sector") %>% 
  ggplot(., aes(x=election, y=estimate))+geom_point()+labs(title="OLS Coefficients of Public Sector on Conservative Vote")+geom_errorbar(aes(ymin=estimate-(1.96*std.error), ymax=estimate+(1.96*std.error)), width=0)+ylim(c(-0.2,0.2))
#ggsave(here("Plots", "sector_conservative_1968_2019_16.png"))

green_models_complete16 %>% 
  unnest(tidied) %>% 
  filter(term=="sector") %>% 
  ggplot(., aes(x=election, y=estimate))+geom_point()+labs(title="OLS Coefficients of Public Sector on Green Vote")+geom_errorbar(aes(ymin=estimate-(1.96*std.error), ymax=estimate+(1.96*std.error)), width=0)+ylim(c(-0.2,0.2))
#ggsave(here("Plots", "sector_greens_1968_2019_16.png"))

#Join all parties and plot sector coefficients
ndp_models_complete16 %>%
  #bind liberal models to ndp models
  bind_rows(., liberal_models_complete16) %>%
  #add conservative models to liberal models
  bind_rows(., conservative_models_complete16) %>%
#  bind_rows(., green_models_complete16) %>%
  unnest(tidied) %>%
  filter(term=="sector"| term=="union_both") %>%
  mutate(term=Recode(term, "'sector'='Sector'; 'union_both'='Union'")) %>%
  ggplot(., aes(x=election, y=estimate, col=vote, alpha=fct_relevel(term, "Union")))+
  geom_point()+
  labs(title="OLS Coefficients of all parties + Greens on PV", alpha="Variable", color="Vote", x="Election", y="Estimate")+
  geom_errorbar(aes(ymin=estimate-(1.96*std.error), ymax=estimate+(1.96*std.error)), width=0)+
  ylim(c(-0.2,0.2))+
  scale_color_manual(values=c("blue", "red", "orange"))+
  facet_grid(rows=vars(vote), switch="y")+geom_hline(yintercept=0, alpha=0.5)+theme(axis.text.x=element_text(angle=90))
...
#save 
ggsave(here("Plots", "M16_Greens_public_sector_all_parties.png"))
